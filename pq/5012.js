...
const geomean = a => Math.pow(
    a.reduce((accu, x) => accu * x),
    1 / a.length
);

const harmmean = a => a.length / a.reduce((accu, x) => accu + 1 / x, 0);

const cubemean = a => Math.cbrt(
    a.reduce((accu, x) => accu + x * x * x, 0) / a.length
);
...
function startFight() {
    ...
    cakeKillTask = tMan.register(() => {
        const mobs =
            map.getAllMonsters()
               .stream()
               .filter(m => m.getId() !== puffDaddy)
