	...
	if (!cm.onQuest()) {
		if (status == 0) {
			if (mode == 0) {
				...
				cm.dispose();
				return;
			} else if (cm.getPlayer().getLevel() >= 120 && cm.getPlayer().getItemQuantity(1902002, true) > 0 && cm.getPlayer().getItemQuantity(1902005, true) == 0) {
		...
		if (status == 0) {
			...
			cm.sendOk(cm.showReward("Thank you so much. #estarts eating#n Oh, and here's a bird I found, if you want it:"));
		} else if (status == 2) {
			cm.rewardPlayer(0, 0);
			cm.gainFame(12);
