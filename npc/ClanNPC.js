...
function action(mode, type, selection) {
    ...
    if (cm.getPlayer().getClan() === -1) {
        if (status === 0) {
            text = "Hey there ! You need a clan to join before doing anything else in this game. ";
            ...
            if (selection === 0) {
                cm.sendNextPrev("There are currently " + cm.getOnlineClanCount(clan) + " in this team.");
            } else if (selection === 1) {
                cm.sendNextPrev("There are currently " + cm.getOfflineClanCount(clan) + " in this team.");
            } else if (selection === 2) {
