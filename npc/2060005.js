...
function action(mode, type, selection) {
	...
	if (cm.getPlayer().getMap().getId() == originalmapid) {
		if (status === 0) {
			if (mode === 0) {
				cm.dispose();
				return;
			}
			if (cm.getQuestStatus(questid) == MapleQuestStatus.Status.COMPLETED && cm.getPlayer().getItemQuantity(1902000, true) == 0) {
				...
				cm.dispose();
				return;
			} else if (cm.getQuestStatus(questid) == MapleQuestStatus.Status.STARTED) {
				cm.sendYesNo("Would you like to go to tame the hog right now?");
			} else if (cm.getQuestStatus(questid) == MapleQuestStatus.Status.COMPLETED && cm.getPlayer().getLevel() >= 100 && cm.getPlayer().getItemQuantity(1902001, true) == 0) {
				var completed = true;
			...
			if (map.playerCount() == 0) {
				...
				map.startPeriodicMonsterDrop(cm.getPlayer(), hog, 60 * 1000, 60 * 60 * 1000);
				cm.dispose();
				return;
			} else {
	...
	} else if (cm.getPlayer().getMap().getId() == mapid) {
		if (status === 0) {
