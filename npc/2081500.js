...
var ids = [6000];
var idon;
var skills = [[[3121006, 30], [3221005, 30], [2321003, 20], [2221005, 30], [2121005, 30], [5220002, 20], [1320008, 25]]];
...
var questions = {
    ...
    "In Marxism, the term \"ideology\" refers to __________________________.": [
        "societal norams which serve to prevent people from challenging the status quo",
        ...
        if (!cm.onQuest()) {
            if (status === 0) {
                if (mode === 0) {
                    ...
                    if (reward.length > 0) {
                        if (p.getMasterLevelById(reward[0]) >= reward[1]) {
                            hasquest = false;
                        }
                    }
                    if (hasquest) {
                        cm.sendSimple(cm.selectQuest(idon, "I can train you to summon powerful monsters on command, to assist you in your journey."));
                    } else {
