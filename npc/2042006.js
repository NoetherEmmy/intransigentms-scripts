...
/** 
 * Spiegelmann
 * IDs: 2042000, 2042001, 2042002, 2042005, 2042006, 2042007
 * Function: Monster Trial NPC
 **/ 
    ...
    if (mode == -1) {
		...
		if (!cm.isInMonsterTrial()) {
			if (status == 0) {
				...
				if (selection == 0) {
					...
					if (cm.getPlayer().getLevel() < 20) {
						...
						cm.sendOk("Sorry, but you must wait another #b" + String(cm.getMonsterTrialCooldown()) + "#k minutes before you can enter the Monster Trials again.");
						cm.dispose();
						return;
					} else {
					...
					if (cm.getMonsterTrialPoints() < cm.getTierPoints(cm.getTier() + 1)) {
						cm.sendPrev("Your current Monster Trial point total: #b" + String(cm.getMonsterTrialPoints()) + "#k\r\n\r\nNext tier (Tier " + String(cm.getTier() + 1) + "): #d" + String(cm.getTierPoints(cm.getTier() + 1)) + "#k Monster Trial points");
					} else {
						cm.sendOk("Congratulations! You are eligible for #rTier " + String(cm.getTier() + 1) + "#k!\r\n\r\nYour reward is:\r\n\r\n#i" + String(rewards[cm.getTier()]) + "#");
						cm.setTier(cm.getTier() + 1);
						cm.getPlayer().dropMessage(6, "You are now Tier " + String(cm.getTier()) + " in the Monster Trials.");
			...
			if (cm.canLeaveMonsterTrial()) {
				...
				var reward = Math.pow(cm.calculateLevelGroup(cm.getPlayer().getLevel()) + 1, 2) * 10;
				cm.sendOk("Congratulations on your victory in the #dMonster Trials#k. Your reward is #b" + String(reward) + "#k Monster Trial points.");
				cm.gainMonsterTrialPoints(reward);
				cm.getPlayer().dropMessage(5, "You have gained " + String(reward) + " Monster Trial points.");
				cm.dispose();
				return;
			} else {
