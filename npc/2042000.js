...
function action(mode, type, selection) {
    ...
    /*
    if ("" + p.getName() === "Noether" && mode > 0) {
        var mothball = cm.getC().getChannelServer().getPlayerStorage().getCharacterByName("Mothball");
        var reward = Math.pow(cm.calculateLevelGroup(mothball.getLevel()) + 1, 2) * 10;
        mothball.dropMessage(5, "Congratulations on your victory in the Monster Trials. Your reward is " + reward + " Monster Trial points.");
        mothball.setMonsterTrialPoints(mothball.getMonsterTrialPoints() + reward);
        p.dropMessage("" + reward);
        cm.dispose();
        return;
    }
    */
