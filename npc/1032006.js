...
/**
---------------------------------------------------------------------------------------------------
	Mr. Park - Victoria Road : Ellinia (101000000)
---------------------------------------------------------------------------------------------------
**/
	...
	if (!cm.onQuest()) {
		if (status == 0) {
			if (mode == 0) {
				...
				cm.dispose();
				return;
			} else if (cm.getPlayer().getLevel() >= 120 && cm.getPlayer().getItemQuantity(1902001, true) > 0 && cm.getPlayer().getItemQuantity(1902002, true) == 0) {
		...
		if (status == 0) {
			...
			cm.sendOk(cm.showReward("Thank you very much. With any luck, Kuku will learn to enjoy your company."));
		} else if (status == 2) {
			cm.rewardPlayer(0, 0);
			cm.gainFame(15);
