...
function action(mode, type, selection) {
    ...
    if (status === 0) {
        if (p.getLevel() >= minLevel) {
            if (
                cm.getQuestStatus(100200) !== Status.COMPLETED &&
                cm.getQuestStatus(100200) !== Status.STARTED
            ) {
                ...
                cm.dispose();
                return;
            } else if (cm.getQuestStatus(100201) === Status.STARTED) {
            ...
            if (cm.getQuestStatus(100201) === Status.COMPLETED) {
