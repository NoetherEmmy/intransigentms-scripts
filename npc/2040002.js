...
function action(mode, type, selection) {
    ...
    if (cm.getQuestStatus(3230).getId() === MapleQuestStatus.Status.STARTED.getId()) {
        if (status === 0) {
            ...
            if (entry === false) {
                ...
                dh.getInstance("DollHouse").registerPlayer(p);
            }
            cm.dispose();
            return;
        }
    } else if (cm.getQuestStatus(3230).getId() === MapleQuestStatus.Status.COMPLETED.getId()) {
